...
std::vector<int>::iterator TND004::stable_partition(std::vector<int>::iterator first,
    ...
    auto it1 = TND004::stable_partition(first, first + mid, p);
    auto it3 = TND004::stable_partition(first + mid, last, p);


    auto it4 = std::rotate(it1, first + mid, it3);
...
void TND004::stable_partition_iterative(std::vector<int>& V, std::function<bool(int)> p) {
    ...
    V.clear();
    std::copy(res.begin(), res.end(), back_inserter(V));
